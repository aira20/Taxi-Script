...
-- ✅ Locations
local locations = {
    penguins = {pos = Vector3.new(4212, 1191, -4352), type = "safe"},
    verdant = {pos = Vector3.new(3785, 84, 1631), type = "safe"},
    guild = {pos = Vector3.new(2545, 82, 1257), type = "safe"},
    fox = {pos = Vector3.new(2047, 112, -348), type = "safe"},
    light = {pos = Vector3.new(878, 267, 680), type = "safe"},
    tower = {pos = Vector3.new(4403, 228, -601), type = "safe"},
    beach = {pos = Vector3.new(1401, 80, 536), type = "safe"},
    rooftop = {pos = Vector3.new(3916, 226, -362), type = "safe"},
    mount = {pos = Vector3.new(4575, 1103, -1734), type = "safe"},
    flare = {pos = Vector3.new(5761, 1143, -3770), type = "safe"},
    tom = {pos = Vector3.new(5635, 245, -66), type = "safe"},
    gym = {pos = Vector3.new(5790, 449, -99), type = "safe"},
    ferry = {pos = Vector3.new(1587, 78, -119), type = "safe"},
    saloon = {pos = Vector3.new(-6122, 118, -1958), type = "safe"},
    cavern = {pos = Vector3.new(3313, 74, -430), type = "safe"},
    bluemoon = {pos = Vector3.new(-8007, 343, -1833), type = "safe"},
    fernhill = {pos = Vector3.new(2324, 84, 655), type = "safe"},
    golden = {pos = Vector3.new(-7012, 108, -1952), type = "safe"},
    copper = {pos = Vector3.new(-5848, 80, -2411), type = "safe"},
    grave = {pos = Vector3.new(3714, 228, -601), type = "safe"},
    polar = {pos = Vector3.new(5130, 1112, -2043), type = "safe"},
    glacial = {pos = Vector3.new(5114, 1124, -2691), type = "safe"},
    butter = {pos = Vector3.new(4029, 226,-458), type = "safe"},
    kingmushroom = {pos = Vector3.new(9362, 524, -31920), type = "safe"},
    moon = {pos = Vector3.new(10161, 5,  -32077), type = "enclosed"},
    crescentshovel = {pos = Vector3.new(9950, -136, -32983), type = "safe"},
    symphony = {pos = Vector3.new(10936, 21, -32917), type = "enclosed"},
}
...
-- Basic teleport for open areas
local function basicTeleport(vehicle, position)
	if vehicle then
		vehicle:PivotTo(CFrame.new(position))
	end
end

-- Safe teleport for enclosed spaces
local function safeTeleport(vehicle, position, player)
	local rayOrigin = position + Vector3.new(0, 100, 0)
	local rayDir = Vector3.new(0, -200, 0)

	local raycastParams = RaycastParams.new()
	raycastParams.FilterDescendantsInstances = {workspace.Active.Vehicles, player.Character}
	raycastParams.FilterType = Enum.RaycastFilterType.Blacklist

	local result = workspace:Raycast(rayOrigin, rayDir, raycastParams)
	local finalPos = result and result.Position + Vector3.new(0, 3, 0) or position + Vector3.new(0, 10, 0)

	if vehicle then
		vehicle:PivotTo(CFrame.new(finalPos))
	end
end

-- Handles teleport based on location type
local function teleportToLocation(player, locationKey)
	local entry = locations[locationKey]
	if not entry then return end

	local vehicle = workspace:FindFirstChild("Active") and workspace.Active:FindFirstChild("Vehicles") and workspace.Active.Vehicles:FindFirstChild(player.Name)
	if not vehicle then return end

	if entry.type == "safe" then
		basicTeleport(vehicle, entry.pos)
	else
		safeTeleport(vehicle, entry.pos, player)
	end
end
